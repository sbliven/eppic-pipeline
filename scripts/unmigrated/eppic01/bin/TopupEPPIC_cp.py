...
class TopupEPPIC:
    ...
    def getPreviousStat(self):
        ...
        PdbCountNew=atof(getoutput("find  /data/dbs/pdb/data/structures/all/mmCIF/ -name *.cif.gz |  wc -l"))
        PdbCount=PdbCountNew-len(self.newPDB)+len(self.deletedPDB)
    ...
    def getShiftsFile(self):
        self.writeLog("INFO: copying latest SIFTS file to UniProt dir")
        cmd="cp -v /data/dbs/uniprot/pdb_chain_uniprot.lst /data/dbs/uniprot/%s/pdb_chain_uniprot.lst"%(self.uniprot)
        chk=getstatusoutput(cmd)
        if chk[0]:
            self.writeLog("ERROR: No SIFTS file available")
            sys.exit(1)
    def sendMessage(self,mailmessage):
        #print mailmessage
        mailcmd="mail -s \"EPPIC topup\" \"eppic@systemsx.ch\" <<< \"%s\""%(mailmessage)
        #mailcmd="mail -s \"EPPIC topup\" \"guido.capitani@psi.ch\" <<< \"%s\""%(mailmessage)
