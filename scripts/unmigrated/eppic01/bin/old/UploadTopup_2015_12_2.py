...
class UploadTopup:
    ...
    def uploadFiles(self):
        uploadcmd="java -jar %s UploadToDb -D eppic_2015_12_2  -d %s/ -f %s/input/pdbinput_%s.list -F  > /dev/null"%(self.eppictoosjar,self.filesDir,self.workDir,self.today)
    ...
    def removeObsolete(self):
        ...
        if self.deletedEntries<20:
            delcmd="java -jar %s UploadToDb -D eppic_2015_12_2 -d %s/ -f %s/input/deletedPDB_%s.list -r  > /dev/null"%(self.eppictoosjar,self.filesDir,self.workDir,self.today)
