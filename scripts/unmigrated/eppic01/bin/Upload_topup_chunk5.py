...
class UploadTopup:
    ...
    def createSymlink(self):
        ...
        for pdb in newPdblist:
            symlinkcmd="cd %s;ln -sf divided/%s/%s"%(self.filesDir,pdb[1:3],pdb)
    ...
    def removeObsolete(self):
        ...
        if self.deletedEntries<20:
            ...
            sendmail="mail -s \"EPPIC topup warning\"  \"guido.capitani@psi.ch\" <<< \"More than 20 obsolete entries found. Please check and delete manually\""
...
ountdiffc,XtalCountdiff,round(XtalCountpdiff,4)))
        ...
        fo.write("\t<tr><td class=\"text\">Total number of chains with UniProt match</td><td class=\"numeric\">%.0f</td><td class=\"numeric\">(%0.2f %%)</td><td class=\"%s\">[%d (%0.4f %%)]</td></tr>\n"%(ChainHasUniprot,ChainHasUnipro
tp,ChainHasUniprotdiffc,ChainHasUniprotdiff,round(ChainHasUniprotpdiff,4)))
        fo.write("\t<tr><td class=\"text\">Total number of chains having at least 10 homologs with 50%% sequence identity</td><td class=\"numeric\">%.0f</td><td class=\"numeric\">(%0.2f %%)</td><td class=\"%s\">[%d (%0.4f %%)]</td></t
r>\n"%(ChainHas10H50P,ChainHas10H50Pp,ChainHas10H50Pdiffc,ChainHas10H50Pdiff,round(ChainHas10H50Ppdiff,4)))
        fo.write("\t<tr><td class=\"text\">Total number of chains having at least 10 homologs with 60%% sequence identity</td><td class=\"numeric\">%.0f</td><td class=\"numeric\">(%0.2f %%)</td><td class=\"%s\">[%d (%0.4f %%)]</td></t
r>\n"%(ChainHas10H60P,ChainHas10H60Pp,ChainHas10H60Pdiffc,ChainHas10H60Pdiff,round(ChainHas10H60Ppdiff,4)))
        fo.write("\t<tr><td class=\"text\">Total number of chains having at least 30 homologs with 60%% sequence identity</td><td class=\"numeric\">%.0f</td><td class=\"numeric\">(%0.2f %%)</td><td class=\"%s\">[%d (%0.4f %%)]</td></t
r>\n"%(ChainHas30H60P,ChainHas30H60Pp,ChainHas30H60Pdiffc,ChainHas30H60Pdiff,round(ChainHas30H60Ppdiff,4)))
        fo.write("\t<tr><td class=\"text\">Total number of chains having at least 50 homologs with 60%% sequence identity</td><td class=\"numeric\">%.0f</td><td class=\"numeric\">(%0.2f %%)</td><td class=\"%s\">[%d (%0.4f %%)]</td></t
r>\n"%(ChainHas50H60P,ChainHas50H60Pp,ChainHas50H60Pdiffc,ChainHas50H60Pdiff,round(ChainHas50H60Ppdiff,4)))
        ...
        for val in Top10Area:
            fo.write("\t<tr><td class=\"text\"><a href=\"http://www.eppic-web.org/ewui/#id/%s\" onclick=\"reloadPage('http://www.eppic-web.org/ewui/#id/%s');\">%s</a></td><td class=\"text\">%s</td><td class=\"numeric\">%.0f</td><td cl
ass=\"numeric\">%0.2f &Aring;<sup>2</sup></td></tr>\n"%(val[0],val[0],val[0],val[1],atof(val[2]),atof(val[3])))
        fo.write("\t</table>\n")
        ...
        for val in Top10Core:
                fo.write("\t<tr><td class=\"text\"><a href=\"http://www.eppic-web.org/ewui/#id/%s\" onclick=\"reloadPage('http://www.eppic-web.org/ewui/#id/%s');\">%s</a></td><td class=\"text\">%s</td><td class=\"numeric\">%.0f</td><t
d class=\"numeric\">%.0f</td></tr>\n"%(val[0],val[0],val[0],val[1],atof(val[2]),atof(val[3])))
        fo.write("\t</table>\n")
        ...
        for val in Top10inter:
                fo.write("\t<tr><td class=\"text\"><a href=\"http://www.eppic-web.org/ewui/#id/%s\" onclick=\"reloadPage('http://www.eppic-web.org/ewui/#id/%s');\">%s</a></td><td class=\"text\">%s</td><td class=\"numeric\">%.0f</td></
...
alue,round(diffper,4)))
	...
	if len(self.runningJobs)==1:
        ...
        mailcmd="mail -s \"EPPIC topup running\" \"guido.capitani@psi.ch\" <<< \"%s\""%(msg2)
    ...
    def sendReport(self):
        ...
        #print mailmessage
        mailcmd2="mail -s \"EPPIC topup finished\" -a \"%s\" \"guido.capitani@psi.ch\" <<< \"%s\""%(self.statFile,mailmessage2)
